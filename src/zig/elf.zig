...
fn init() modules.InitError!virtual.Vregion {
    const elf_tag = multiboot.multibootFindTag(
        multiboot.MultibootTagELFSections,
    ) catch return error.Unsupported;
    var start: ?usize = null;
    var end: ?usize = null;
    var offset: usize = 0;
    while (offset < elf_tag.num * elf_tag.entsize) : (offset += elf_tag.entsize) {
        const h: *align(1) ELFSectionHeader = @ptrFromInt(
            @intFromPtr(&elf_tag.first_section) + offset,
        );
    ...
    const start_ptr: [*]align(paging.page_size) u8 = @ptrFromInt(
        paging.page_align.forward(start_res),
    );
    return .{
        .vaddr = start_ptr,
        .paddr = start_ptr,
...
pub var mod: modules.Module(virtual.Vregion) = .{
    .name = "elf",
    .init = init,
};
